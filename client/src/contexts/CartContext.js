...
import { useAuth } from './AuthContext'; // To react to login/logout
...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.getCart(); // API call to get the cart
      const cartData = response.data.data.cart;
      if (cartData) {
        setCartItems(cartData.items || []);
        setCartId(cartData.id);
        logger.info(`CartContext: Cart fetched successfully. ID: ${cartData.id}, Items: ${cartData.items?.length || 0}.`);
      } else {
        ...
        logger.info('CartContext: No active cart found on backend, initialized empty cart locally.');
      }
    } catch (err) {
   ...
   */
  useEffect(() => {
    if (!authLoading) { // Only proceed once authentication status is determined
      logger.debug(`CartContext: Auth status resolved (isAuthenticated: ${isAuthenticated}). Triggering fetchCart.`);
      fetchCart();
    } else {
  ...
  const updateCartStateFromResponse = (responseData) => {
    ...
    if (cartData) {
      ...
      logger.warn('CartContext: Cart operation response did not contain full cart data. Refetching cart.');
      fetchCart();
    }
  };
  ...
  const addToCart = useCallback(async (productId, quantity = 1) => {
    ...
    try {
      logger.debug(`CartContext: Adding to cart - Product ID: ${productId}, Quantity: ${quantity}.`);
      const response = await api.addItemToCart(productId, quantity);
      updateCartStateFromResponse(response.data);
      logger.info(`CartContext: Product ${productId} (Qty: ${quantity}) added to cart successfully.`);
      return { success: true, message: "Item added to cart!" };
    } catch (err) {
      const message = err.response?.data?.message || 'Could not add item to cart. Please try again.';
      logger.error('CartContext: Failed to add item to cart.', message, err);
      setError(message);
      return { success: false, message };
    } finally { setLoading(false); }
  }, [isAuthenticated, fetchCart]); // fetchCart included if updateCartStateFromResponse uses it
  ...
  const updateCartItemQuantity = useCallback(async (itemId, quantity) => {
    ...
    if (quantity < 1) { // If new quantity is 0 or less, remove the item instead
  ...
  }, [isAuthenticated, fetchCart]); // fetchCart dependency
  ...
  const clearCart = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await api.clearCart(); // API call to clear cart on the backend
  ...
  const contextValue = useMemo(() => ({
    ...
    cartId, cartItems, cartTotals.itemCount, cartTotals.subtotal, loading, error,
    fetchCart, addToCart, updateCartItemQuantity, removeCartItem, clearCart
  ]);
...
export const useCart = () => {
  ...
  if (context === null) {
    throw new Error('useCart must be used within a CartProvider. Ensure your component is wrapped by CartProvider.');
  }
  return context;
};