...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const verifyAuthStatus = async () => {
      ...
      if (currentToken) {
        ...
        api.defaults.headers.common['Authorization'] = `Bearer ${currentToken}`;        try {          // First verify the token
          await api.verifyAuthToken(); // Verify token is valid
          // Then get the user profile
          const profileResponse = await api.getCurrentUserProfile();
          const userData = profileResponse.data.data.user;
          setUser(userData);
          setToken(currentToken);
          logger.info(`AuthContext: User ${userData.email} authenticated successfully via stored token.`);
        } catch (error) {
   ...
   */  const login = useCallback(async (credentials) => {
    ...
    try {      // Attempt login
      ...
      // Store token and update axios default header
      localStorage.setItem('authToken', newToken);
      api.defaults.headers.common['Authorization'] = `Bearer ${newToken}`;
        // Update state
      setUser(userData);
      setToken(newToken);
      logger.info(`AuthContext: User ${userData.email} logged in successfully.`);
      setLoading(false);
      return { success: true };
    } catch (error) {
