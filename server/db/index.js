...
if (config.use_env_variable && process.env[config.use_env_variable]) {
  logger.info(`Connecting to database using environment variable: ${config.use_env_variable}`);
  ...
  sequelize = new Sequelize(config.database, config.username, config.password, config);
    ...
    logger.error('Database configuration is incomplete. Provide DATABASE_URL or DB_NAME/DB_USER.');
    process.exit(1);
}
